...
(function () {
  ...
  function render() {
    ...
    $('#schedule').innerHTML = `
      ...
      ${dayShifts.length ? dayShifts.map((s) => { const p = Store.position(s.positionId); const loc = Store.location(s.locationId); const e = Store.entryForShift(s.id); return `<div class="card" data-reveal><div class="shift-card"><div class="time" style="background:${p.color}14;color:${p.color}">${UI.hmLabel(s.start).replace(' ', '<small>')}</small></div><div class="grow"><div class="t">${esc(s.label)} shift</div><div class="d">${range(s.start, s.end)} · ${hours(Store.shiftHours(s), 0)}</div><div class="tags"><span class="tag">${icon('location-outline')}${esc(loc.name)}</span><span class="tag">${icon('briefcase-outline')}${esc(p.name)}</span>${!s.published ? '<span class="tag" style="color:var(--amber-600);background:var(--amber-50)">Draft</span>' : ''}${e && e.clockIn ? `<span class="tag" style="color:var(--green-600);background:var(--green-50)">${icon('checkmark-circle-outline')}${e.clockOut ? hours(Store.entryHours(e)) + ' logged' : 'On shift'}</span>` : ''}</div></div></div></div>`; }).join('') : `<div class="card" data-reveal><div class="empty">${icon(off ? 'airplane-outline' : 'cafe-outline')}<div class="t">${off ? UI.leaveType(off.type).label + ' · approved' : 'Day off'}</div><div>${off ? 'Enjoy your time off!' : 'Nothing scheduled on this day.'}</div></div></div>`}
