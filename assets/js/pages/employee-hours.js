...
(function () {
  ...
  function render() {
    ...
    $('#hours').innerHTML = `
      ...
      <div class="card" data-reveal><div class="list">${entries.length ? entries.slice().reverse().map((e) => { const st = Store.entryState(e); const brk = e.breakStart && e.breakEnd ? Math.round((new Date(e.breakEnd) - new Date(e.breakStart)) / 60000) : 0; return `<div class="entry-row"><div class="date"><b>${Store.parseDate(e.date).getDate()}</b><span>${UI.dayName(e.date)}</span></div><div><div class="t">${time(e.clockIn)} – ${e.clockOut ? time(e.clockOut) : '<span style="color:var(--green-600)">now</span>'}</div><div class="d">${brk ? `${brk} min break` : 'No break'}${st === 'on' || st === 'break' ? ' · <b style="color:var(--green-600)">On shift</b>' : ''}</div></div><div class="h">${hm(Store.entryHours(e))}${e.source === 'kiosk' ? '<small>kiosk</small>' : ''}</div></div>`; }).join('') : `<div class="empty">${icon('time-outline')}<div class="t">No hours logged</div><div>Clock in at the kiosk and your hours appear here.</div></div>`}</div></div>
